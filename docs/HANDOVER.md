...
## 13. 2026-09-16 页面级容器保活管理、模型与引擎授权及用户体系增强

### 1. 页面级容器生命周期与保活管理
- **系统设置持久化**：新增 `system_settings` 数据表与 `/api/system-settings` 接口，支持在界面“执行引擎”管理页中直接修改容器闲置销毁时间（秒）。
- **即时生效**：前端表单输入后保存，直接更新数据库并在 `ContainerAgentRunner` 运行时实时动态读取，无需再改动环境变量或重启后端服务；若设为 0，单次任务执行完毕即刻销毁。

### 2. 模型与执行引擎的角色授权控制
- **数据表与接口增强**：
  - `execution_engines` 表新增 `allowed_roles` 字段（默认为 `'admin,user'`）；
  - `model_configs` 表新增 `allowed_roles` 字段（默认为 `'admin,user'`）。
- **管理员控制台**：管理员可在“执行引擎”与“模型设置”表单中自由配置该引擎或模型是“管理员 + 普通用户”可用，还是“仅管理员可见与使用”。
- **权限拦截与视图隔离**：
  - 列表接口（`/api/models` 与 `/api/execution-engines`）按当前用户身份过滤不可见配置；
  - 任务下发（`/api/tasks`）严格校验角色授权，若普通用户越权调用未授权模型或第三方执行引擎，服务端返回 403 明确拒绝。

### 3. 用户体系与项目协作说明
- **普通用户能力**：普通用户登录后具备完整的工作台对话、历史会话查看、项目文件操作以及通过成员管理共享项目（`/api/workspaces/{id}/members`）的权限；仅管理员可进入“用户管理”、“执行引擎”、“模型设置”进行底层资源配置与授权分发。
