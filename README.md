...
AgentWeave（中文名“智织”）是一个现代化的多智能体与第三方执行引擎协同调度平台。它不仅提供智能体、专家团、Skill 与 MCP 工具的统一管理，更深度集成了 **Codex**、**Claude Code** 等专业代码与终端智能体引擎。平台采用“**用户/项目物理路径隔离 + 独立 Docker 容器沙箱 + 挂载依赖持久化 + 暖容器延迟回收**”架构，确保不同用户、不同项目之间的执行环境零污染，同时兼顾冷启动响应速度与连续多轮会话的上下文恢复。
